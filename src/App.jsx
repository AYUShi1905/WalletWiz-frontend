...
const Layout = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-zinc-950 text-slate-800 dark:text-slate-100 flex items-center justify-center p-0 sm:p-4">
      {/* Phone/Card Frame Wrapper */}
      <div className="w-full max-w-md h-screen sm:h-[88vh] sm:max-h-[850px] bg-slate-50 dark:bg-brand-dark flex flex-col relative overflow-hidden sm:rounded-3xl sm:border sm:border-slate-200/80 sm:dark:border-zinc-900/80 sm:shadow-2xl">
        {/* Full-Width Top Header (Clean boundary to hide scrolled content) */}
        <header className="flex items-center justify-between h-16 px-4 bg-white/95 dark:bg-zinc-950/95 border-b border-slate-200 dark:border-zinc-900 backdrop-blur-md absolute top-0 left-0 right-0 z-30 shadow-sm sm:rounded-t-3xl">
        ...
        <main className="flex-1 w-full pt-20 pb-24 px-4 bg-slate-50 dark:bg-brand-dark overflow-y-auto">
          <Outlet />
        </main>

        {/* Full-Width Pinned Bottom Navigation Bar */}
        <nav className="flex items-center justify-between h-16 bg-white/95 dark:bg-zinc-950/95 border-t border-slate-200 dark:border-zinc-900 absolute bottom-0 left-0 right-0 z-30 px-10 shadow-lg sm:rounded-b-3xl">
          <NavLink
            to="/"
            className={({ isActive }) =>
